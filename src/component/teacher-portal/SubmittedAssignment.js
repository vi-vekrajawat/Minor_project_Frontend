    ...
    <div style={{ width: "100vw", minHeight: "100vh", overflowX: "hidden", backgroundColor: "#f8f9fa" }}>
      ...
      <div className="submitted-header">
        <div>
          ...
          <Link to="/submitted" className="text-white" style={{ marginLeft: "15px", textDecoration: "none" }}>Submitted Assignment</Link>
        </div>
        <div>
      ...
      <div style={{ display: "flex", width: "100vw", minHeight: "calc(100vh - 60px)" }}>
        ...
        <div style={{ flexGrow: 1, padding: "20px" }}>
          ...
          <div className="filters">
            <div>
              <label>Day:</label>
              <select className="form-control" value={filterDay} onChange={(e) => setFilterDay(e.target.value)}>
            ...
            <div>
              ...
              <select className="form-control" value={filterBatch} onChange={(e) => setFilterBatch(e.target.value)}>
                <option value="all">All</option>
                {batchOptions.map((batch,i) => batch && <option key={i} value={batch}>{batch}</option>)}
              </select>
            </div>
          </div>
                  ...
                  {filteredTasks.map((a) => {
                    ...
                    return (
                      <tr key={a._id}>
                        ...
                        <td>{a.feedback}</td>
                        <td>{a.status}</td>
                        <td>{date ? `${date.toLocaleDateString()} (${date.toLocaleDateString("en-US",{ weekday: "long" })})` : "-"}</td>
