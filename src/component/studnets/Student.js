...
function Student() {
    ...
    const loadAssignmnets = async () => {
        try {
            ...
            const allTasks = response.data.assignment

            //  Batch-wise filter (match batchId with user.batch)
            const filteredTasks = allTasks.filter(task => task.batchId === user.batch)
            ...
            <div>
                <div className="d-flex bg-primary text-white p-2">
                    <div className="d-flex">
                        <div>ITEP</div>
                        <Link style={{ textDecoration: "none", color: "inherit" }} className="ml-3">Dashboard</Link>
                        <Link to="/submission" style={{ textDecoration: "none", color: "inherit" }} className="ml-3">My Assignment</Link>
                        <Link to="/student-profile" style={{ textDecoration: "none", color: "inherit" }} className="ml-3">Profile</Link>
                    </div>
                    ...
                    <div className="ml-5">
                        ...
                        <div className="mt-3 p-2" style={{ boxShadow: "0px 0px 3px 0px grey", maxHeight: "350px", overflowY: "auto" }}>
                            <div>
                                ...
                                <div className="mt-3">
                                    {task.map((data, index) => (
                                        <div key={index} className="mt-3 p-1" style={{ border: "0.1px solid grey" }}>
                                            ...
                                            {data?.file && (
                                                <small>
                                                    ...
                                                    >
                                                        <i className="fa fa-download" style={{ marginRight: "5px" }}></i>Download File
                                                    </a>
                                                </small>
                                            )}<br />

                                            <Link to="/submission" className="btn btn-success">Submit</Link>
